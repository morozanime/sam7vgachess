...
// Initialize the board and go to the
...
//				printf("%c%c -> %c%c\n", from[1] + 'a', from[0] + '1', to[1] + 'a', to[0] + '1');
		...
		// CPU move.
			...
			for (i = 0; i < 8; ++i) {
				for (j = 0; j < 8; ++j) {
					if (board[i][j] & 0x08) { // Get possible moves.
						cpufrom[0] = i;
						cpufrom[1] = j;
						p = board[i][j];
						...
						if (((board[cpufrom[0]][cpufrom[1]] & 0x07) == lastmove) && (movetype < 3)) { // Prevent consistent moving of the same piece.
							piecerate -= 3;
						}
...
// of the best move.
{
	...
	for (i = 0; i < 8; ++i) {
		for (j = 0; j < 8; ++j) {
			if (currmovemask[i] & 0x01) { // Test a Valid Move.
				to[0] = i;
				to[1] = j;
				if (test_singlemove(from, to, board[from[0]][from[1]])) { // At this point the validmovemask and capturemask
					...
					;
					movetype = NORMAL;
					if ((validmovemask[from[0]] >> from[1]) & 1) { // Piece can be taken - add some weight to escape.
						tmp = (board[from[0]][from[1]] & 0x07) / 2;
					}

					if ((!((capturemask[i] >> j) & 1)) && ((currcapturemask[i] >> j) & 1)) { // Best Case Scenario - We can Capture a piece and cannot be
																							 // captured in turn.
						movetype = SAFECAPTURE;
					} else if ((currcapturemask[i] >> j) & 1) { // Can Capture a piece.
						movetype = CAPTURE;
					} else if (!((capturemask[i] >> j) & 1)) { // Safe move.
						movetype = SAFE;
					}
					...
					if (tmp > piecerate) { // A better move that any so far with this piece.
						piecerate = tmp;
						bestmove = move;
					}
					tmp = 0;
				}
				for (k = 0; k < 8; ++k) { // Reset Masks.
					validmovemask[k] = 0;
					capturemask[k] = 0;
				}
			}
...
// Return true if move ok, false otherwise.
{
	...
	kingcapture = FALSE;
	i = j = 0;
	while ((i < 8) && (!kingcapture)) { // scan rows.
		while ((j < 8) && (!kingcapture)) { // scan columns.
			if (is_opp_piece(i, j, (p & 0x08))) { // Opponents Piece - Get valid moves.
...
// Update the panel with a move.
{
	...
	if (piecetype(p) == KING) { // King Move.
		updatekingpos(to, (p & 0x08));
		panel_draw(from[0], from[1], EMPTY);
		panel_draw(to[0], to[1], p);
		board[from[0]][from[1]] = EMPTY;
		board[to[0]][to[1]] = p;
		if (((to[1] - from[1]) != 1) && ((from[1] - to[1]) != 1)) { // Castle Move.
	...
	} else if (((p == 0x09) && (to[0] == 0)) || ((p == 1) && (to[0] == 7))) { // Queening a pawn.
		panel_draw(from[0], from[1], EMPTY);
	...
	if ((from[0] == baserow) && ((*basemask) & (1 << from[1]))) { // Reflect any changes in the base row. After a Castle this
																  // becomes superflous.
		(*basemask) &= ~(1 << from[1]);
	}
}
